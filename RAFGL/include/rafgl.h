...
void rafgl_raster_draw_spritesheet2(rafgl_raster_t *raster, rafgl_spritesheet_t *spritesheet, int sheet_x, int sheet_y, int x, int y);
...
int rafgl_raster_draw_raster2(rafgl_raster_t *to, rafgl_raster_t *from, int x, int y);
...
void rafgl_raster_draw_spritesheet2(rafgl_raster_t *raster, rafgl_spritesheet_t *spritesheet, int sheet_x, int sheet_y, int x, int y)
{
    ...
    for(yi = fuc; yi < fdc; yi++)
    {
        for(xi = flc; xi < frc; xi++)
        {
            ...
            if(sampled.rgba != RAFGL_COLOUR_KEY.rgba)
            {
                if(sampled.r <= 250 || sampled.g <= 250 || sampled.b <= 250){
                    pixel_at_pm(raster, xi, yi) = sampled;
                }
            }
        }
...
int rafgl_raster_draw_raster2(rafgl_raster_t *to, rafgl_raster_t *from, int x, int y)
{
    ...
    for(yi = fuc; yi < fdc; yi++)
    {
        for(xi = flc; xi < frc; xi++)
        {
            ...
            if(sampled.rgba != RAFGL_COLOUR_KEY.rgba)
            {
                if(sampled.r <= 245 || sampled.g <= 245 || sampled.b <= 245){
                    pixel_at_pm(to, xi, yi) = sampled;
                }
            }
        }
